...
BUILTIN_RULES: tuple[Rule, ...] = (
    ...
    _rule("delete_git_reset_hard", RiskLevel.DELETE, Decision.REQUIRE_APPROVAL, "git hard reset discards workspace changes",
          r"re:^\s*git\s+reset\b.*\s--hard\b"),
    ...
    _rule("network_download_or_remote", RiskLevel.NETWORK, Decision.REQUIRE_APPROVAL, "network access detected",
          r"re:^\s*(curl|wget)\b", r"re:^\s*git\s+(clone|fetch|pull|push)\b", r"re:^\s*(ssh|scp|rsync)\b", r"re:^\s*docker\s+pull\b"),
    _rule("network_package_install", RiskLevel.NETWORK, Decision.REQUIRE_APPROVAL, "package installation may access the network",
          r"re:^\s*(python\s+-m\s+)?pip\s+install\b", r"re:^\s*npm\s+install\b", r"re:^\s*poetry\s+install\b", r"re:^\s*uv\s+pip\s+install\b"),
    _rule("network_publish_or_api", RiskLevel.NETWORK, Decision.REQUIRE_APPROVAL, "publish or remote API command detected",
          r"re:^\s*(gh|hub)\b", r"re:^\s*npm\s+publish\b", r"re:^\s*twine\s+upload\b"),
